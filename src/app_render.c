...
/*   Updated: 2025/09/09 19:02:52 by csamaha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	update_animations(t_app *app)
{
	...
	gettimeofday(&v, NULL);
	now = (long)v.tv_sec * 1000L + (long)(v.tv_usec / 1000L);
	dt_ms = now - app->last_anim_time_ms;
	if (dt_ms < 0)
		dt_ms = 0;
	app->last_anim_time_ms = now;
	aa = app->torch_anim_accum_ms + dt_ms;
	app->torch_anim_accum_ms = aa;
	while (app->torch_anim_accum_ms >= app->torch_frame_ms)
	{
		app->torch_anim_accum_ms = app->torch_anim_accum_ms
			- app->torch_frame_ms;
...
int	render_loop(void *param)
{
	...
	update_player(app);
	cc = app->cfg.ceiling.argb;
	fc = app->cfg.floor.argb;
	draw_background(&app->frame, cc, fc);
	raycast_frame(app);
	draw_sprites(app);
	draw_minimap(app);
